...
export default function Profile() {
  ...
  const settings = [
    ...
    { icon: Crown, label: 'Special Plans', sub: (() => { try { const { getActivePlan, getPlanProgress, getPlanById } = require('@/lib/event-plan-service'); const ap = getActivePlan(); if (ap) { const p = getPlanProgress(); const m = getPlanById(ap.planId); return `${m?.name || 'Active'} — Day ${p?.dayNumber}/${p?.totalDays}`; } } catch {} return 'Browse transformation plans'; })(), action: () => navigate('/planner?tab=Plans') },
