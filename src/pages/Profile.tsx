...
export default function Profile() {
  ...
  const settings = [
    ...
    { icon: Crown, label: 'Special Plans', sub: (() => { const ap = getActivePlan(); if (ap) { const p = getPlanProgress(); const m = getPlanById(ap.planId); return `${m?.name || 'Active'} — Day ${p?.dayNumber}/${p?.totalDays}`; } return 'Browse transformation plans'; })(), action: () => navigate('/planner?tab=Plans') },
