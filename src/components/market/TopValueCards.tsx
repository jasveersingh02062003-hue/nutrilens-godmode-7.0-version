      ...
      <div className="grid grid-cols-3 gap-2">
        {items.slice(0, 3).map((item, i) => (
          ...
          >
            {/* Rank medal */}
            {i === 0 && (
              <div className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-yellow-500/20 flex items-center justify-center">
                <span className="text-[10px]">🥇</span>
              </div>
            )}
            {i === 1 && (
              <div className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-gray-300/20 flex items-center justify-center">
                <span className="text-[10px]">🥈</span>
              </div>
            )}
            {i === 2 && (
              <div className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-amber-700/20 flex items-center justify-center">
                <span className="text-[10px]">🥉</span>
              </div>
            )}
