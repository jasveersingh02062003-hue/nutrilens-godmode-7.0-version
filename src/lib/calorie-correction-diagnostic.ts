...
import { computeAdjustmentMap, computeSafeSpreadDays, computeMaxDailyAdjustment, type DailyBalanceEntry, type CorrectionMode } from './calorie-correction';
...
const TEST_MODE: CorrectionMode = 'balanced';
const TEST_TDEE = 2000;
...
export function runTest1(): TestResult {
  ...
  const maxAdj = computeMaxDailyAdjustment(TEST_TDEE, TEST_MODE);
  const adjMap = computeAdjustmentMap(pastLogs, baseTarget, TEST_TDEE, TEST_MODE);
  ...
  // Clamp: no single day exceeds mode cap
  const allClamped = Object.values(adjMap).every(v => Math.abs(v) <= maxAdj);
  const passed = conserved && allClamped;

  return {
    name: 'Test 1 – Single Surplus Day (700 kcal)',
    passed,
    expected: `Conservation (surplus ${surplus} + adj ≈ 0), all days ≤ ±${maxAdj}`,
...
export function runTest2(): TestResult {
  ...
  return {
    ...
    passed,
    expected: `Positive adjustments, conserved, all ≤ ±${maxAdj}`,
...
export function runTest3(): TestResult {
  ...
  const maxAdj = computeMaxDailyAdjustment(TEST_TDEE, TEST_MODE);
  const adjMap = computeAdjustmentMap(days, baseTarget, TEST_TDEE, TEST_MODE);
  const totalAdj = Object.values(adjMap).reduce((s, v) => s + v, 0);
  ...
  return {
    ...
    passed,
    expected: `Conservation, all days ≤ ±${maxAdj}, spread ≥ 2 days`,
